...
const MessageItem: React.FC<MessageItemProps> = ({ message, isOwn }) => {
    ...
    if (isImage() && !imageError) {
      return (
        <div className="image-container relative group">
          {/* Loading skeleton */}
          {!imageLoaded && (
            <div className="w-full h-32 bg-white/[0.05] animate-pulse rounded-lg flex items-center justify-center">
          ...
          <img
            ...
            className={`max-w-full h-auto rounded-lg border ${
              isOwn ? 'border-white/30' : 'border-white/10'
  ...
  return (
    <>
      <div className={`${isImage() ? 'max-w-[75%]' : 'max-w-[75%] md:max-w-[60%]'} p-3 rounded-xl ${isOwn
        ? 'bg-gradient-to-br from-[#7B61FF] to-[#5B47CC] text-white shadow-lg shadow-[#7B61FF]/20'
        : 'bg-white/[0.08] backdrop-blur-sm border border-white/[0.1] text-white'
        }`}>
        <div className="message-content">
          {renderContent()}
        </div>
        <div className={`text-[11px] mt-1.5 ${isOwn ? 'text-white/60' : 'text-gray-500'}`}>
